...
import {SetPerson, LoadPerson, LoadPersonMeta} from '../../people/state/person-state-actions';
...
export class SearchInputComponent implements OnInit {
    ...
    private openMediaItemPage(response: any, mediaItem: Title|Person) {
        if (mediaItem.type === MEDIA_TYPE.PERSON) {
            // console.log( new SetPerson(response as GetPersonResponse) );
            let personId = response.person.id ? +response.person.id : 0;
            // this.store.dispatch(new LoadPersonMeta(personId));
            this.store.dispatch(new LoadPerson(+personId))
            // .pipe(
            //     catchError(() => {
