import {
    Component,
    OnInit,
    ViewEncapsulation,
    ChangeDetectionStrategy,
    ElementRef,
    Input,
    OnDestroy, ViewChild
} from '@angular/core';
import * as noUiSlider from 'nouislider';
import {ControlValueAccessor, NG_VALUE_ACCESSOR} from '@angular/forms';
import {BehaviorSubject, Subject} from 'rxjs';

@Component({
    selector: 'single-range-slider',
    templateUrl: './single-range-slider.component.html',
    styleUrls: ['./single-range-slider.component.scss'],
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    providers: [{
        provide: NG_VALUE_ACCESSOR,
        useExisting: SingleRangeSliderComponent,
        multi: true,
    }]
})
export class SingleRangeSliderComponent implements OnInit, ControlValueAccessor, OnDestroy {
    @ViewChild('sliderEl', {static: true}) sliderEl: ElementRef<HTMLDivElement>;
    @Input() min = 1;
    @Input() max = 100;
    @Input() step = 1;
    @Input() float = false;
    @Input() affix;

    private propagateChange: Function;
    private slider: noUiSlider.noUiSlider;
    public value$: BehaviorSubject<(string|number)[]> = new BehaviorSubject([this.min, this.max]);

    ngOnInit() {
        this.slider = noUiSlider.create(this.sliderEl.nativeElement, {
            start:this.min,
            connect:[true,false],
            step: this.step,
            range: {
                min: this.min,
                max: this.max
            },
        }) as any;

        this.slider.on('change', sliderValue => {
            const value = this.formatValue(sliderValue);            
            this.propagateChange(value[0]);
            this.value$.next(value);
        });

        this.slider.on('update', sliderValue => {
            const value = this.formatValue(sliderValue);
            this.value$.next(value);
        });
    }

    private formatValue(value: string[]) {
        return value.map(val => {
            return this.float ? parseFloat(val).toFixed(1) : parseInt(val).toFixed(0);
        });
    }

    ngOnDestroy() {
        this.slider.destroy();
    }

    public writeValue(value: number[]) {
        if ( ! this.slider) return;
        if ( ! value) value = [this.min, this.max];
        this.slider.set(value);
    }

    public registerOnChange(fn: Function) {
        this.propagateChange = fn;
    }

    public registerOnTouched() {}
}
